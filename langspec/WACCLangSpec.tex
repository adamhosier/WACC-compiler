...
Variable names must not clash with WACC keywords or variables that have been previously declared using the same identifier. 

They can consist of letters, numbers and the underscore character, however, a variable's identifier cannot begin with a number.
...
A variable must be declared before being accessed or passed around as a parameter.

Any attempt to access an undeclared variable results in a compilation error (semantic).
...
In this case the redefinition only holds within the child scope. We must adhere to the type that was specified in the redefinition, as it does not have the same definition as the variable defined in the parent scope. 

Once the child scope is exited, the redefinition of a variable is destroyed and its definition reverts back to that of the parent scope.

\fillgap{DONE}{5 marks}
...
For example the statement \lit{exit -1} will be wrapped to \lit{exit 255}.
The integer expression matches with a simple convention of exit codes:
\begin{itemize}
  \item \lit{0}: Success
  \item \lit{100}: Syntax Error
  \item \lit{200}: Semantic Error
  \item \lit{255} or \lit{-1}: Common General Error
...
\begin{table}
  ...
  \begin{tabulary}{\textwidth}{C|L|C|C}
    ...
    \hline
    \lit*{string} or \lit*{char[]} & Outputs each character in the array in succession & ``chair'' & ``chair'' \\
    \hline
    Other Array Types & Printing an array variable gives an address & [0, 1, 2] & \#addrs\# \\  
    \hline
    \lit*{pair} & Printing a pair gives an address & \lit*{newpair(a, b)}\footnotemark[1] & \#addrs\# \\
    \hline
  \end{tabulary}
...
\fillgap{DONE}{6 marks}
A while loop statement allows a body of code to be executed repeatedly given a \lit{bool} condition.
The while loop's body is denoted by \lit{do} and closed with \lit{done}. 
The execution of the while loop's body only terminates when the \lit{bool} given to the statement evaluates to \lit{false}.  
...
\fillgap{DONE}{2 marks}
Sequention composition is the act of joining two or more statements together with the use of a semicolon (\lit{;}).
As a statement \synt{stat} can itself be a composition of two different statements, is it possible to compose together long chains of instructions.
When running, the second statement is executed only after the first statement has finished.
It is conventional, but not essential, to put each statement on a new line, finishing each line with the semicolon to compose them.
It is importnat to note that the final statement in a composition does not require a semicolon, and adding one will cause a syntax error.
...
The expressions of the WACC language have been chosen to be side-effect free. This means that there are no expressions that when declared modify existing data.
...
\begin{table}
  ...
  \begin{tabular}{cclc}
    ...
    \lit*{\char`\\ t} & \lit*{0x09} & tab & TAB \\
    \lit*{\char`\\ n} & \lit*{0x0a} & new line & LF \\
...
\begin{table}
  ...
  \begin{tabulary}{\textwidth}{CCCL}
    ...
    \lit{chr} & \lit*{int} & \lit*{char} & Inverse Ordinal Value \\
    \hline
  \end{tabulary}
...
\item The \lit{ord} operator returns the ASCII \lit{int} representation of the given \lit{char} parameter. \fillgap{DONE}{1 mark}

\item The \lit{chr} operator returns the \lit{char} corresponding to the given \lit{int} under ASCII representation. \fillgap{DONE}{1 mark} 
...
\fillgap{DONE}{3 marks}
Comments are entries into the source code that provide no functional meaning.
A program with no comments, will behave exactly the same as the same program full of valid comments.
A valid comment is given by the \lit{\#} character followed by any non-EOL symbol, ending at the end of a line.
Commends can be started anywhere in the code, including after a sequential combinatoin symbol (\lit{;}), but any code written between a \lit{\#} symbol, and the end of that line, is not executed, and discarded by the compiler.
